<script context="module">
  ...
  export const handleImgFiles = (files,type="img") => {
    if(files.length === 0){
      return
    }
    const blobURL = URL.createObjectURL(files[0])
      get(midiList).custom.img = blobURL
      get(midiList).custom.type = type
...
<container>
  ...
  <div class="setting">
    ...
    <label class="upload {$midiList.custom.url ? 'selected':''}" for="uploadImage">
      ...
      <input type="file" id="uploadImage" name="fileList" accept=".midi,.jpg,jpeg,png,mp4,.mov" on:change={handleClickUpload}>
    </label>
  </div>
...
<style>
  ...
  h3 {
    ...
    margin: 0 0px 0 14px;
    border-radius: 188px;
    font-variation-settings: "wght" 72, "wdth" 133, "ital" 1;
    display: flex;
    align-items: center;
    font-weight: normal;
  }
