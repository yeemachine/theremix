<script>
  ...
  thereminMobile.addChild(base, symbols, knob_left, knob_right, switchRight);

  var dirLight = new PIXI.lights.DirectionalLight(0xffffff, 0.3, base);
  ...
  $: {
    ...
    knob_left.scale = {
      x: 1 + 0.4 * $tweenKnobLeft,
      y: 1 + 0.4 * $tweenKnobLeft,
    };
    knob_left.x = base.width * 0.236;
    knob_left.y = base.height * 0.227;
    knob_left.interactive = $active ? true : false;

    knob_light.x = base.x + base.width * 0.5;
    knob_light.y = base.y + base.height * 0.5;
    knob_light.brightness = 0.4 + 0.4 * $backOut0_1;

    //Knob Right Positioning
    knob_right.scale = {
      x: 1 + 0.4 * $tweenKnobRight,
      y: 1 + 0.4 * $tweenKnobRight,
    };
    knob_right.x = base.width * 0.5095;
    knob_right.y = base.height * 0.227;
    knob_right.interactive = $active ? true : false;
    symbols.scale = {
      x: 0.5 + 0.6 * $tweenKnobRight,
      y: 0.5 + 0.6 * $tweenKnobRight,
    };
    ...
    switchRight.x = base.width * 0.77;
    switchRight.y = base.height * 0.226;
